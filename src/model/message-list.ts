...
export class MessageList {
  ...
  readMessagesPromise: Promise<any> | null;
  stopTimer: NodeJS.Timeout | null;
  markTimer: NodeJS.Timeout | null;

  onMessage = new Signal();
  onDeleteMessage = new Signal();
  onModifyMessage = new Signal();
  onUpdateReadState = new Signal();
  onUpdateMentions = new Signal();

  constructor(account: Account, type: string, id: string) {
  ...
  markUnread() {
    if (!this.isRead || this.isDisplaying) return;
    this.isRead = false;
    this.onUpdateReadState.dispatch();
  }
  ...
  select() {
    ...
    if (this.messagesReady) this.markRead();
  }

  deselect() {
    if (--this.viewerCount === 0) this.isDisplaying = false;
  }
  ...
  stopReceiving() {
    if (--this.receiverCount === 0)
      this.stopTimer = setTimeout(
        this.stopReceivingImpl.bind(this),
  ...
  findMessageIndex(id: string, timestamp: number) {
    if (!this.hasMessages()) return -1;
    const date = new Date(timestamp * 1000);
    for (let i = this.messages.length - 1; i >= 0; --i) {
      const message = this.messages[i];
      if (message.id === id) return i;
      if (message.date < date) return -1;
    }
    return -1;
  }

  findMessage(id: string, timestamp: number) {
  ...
  async dispatchMessage(message: Message) {
    ...
    if (this.isDisplaying) {
      ...
      this.markUnread();
      if (!this.firstUnreadTs) this.firstUnreadTs = this.latestTs;
    ...
    if (this.hasMessages()) this.compareMessage(message, this.latestMessage());
  ...
  deleteMessage(id: string, timestamp: number) {
  ...
  modifyMessage(id: string, timestamp: number, modified: {}) {
    if (!this.isReceiving) return;
    const i = this.findMessageIndex(id, timestamp);
    if (i === -1) return;
  ...
  async setMessageStar(id: string, timestamp: number, hasStar: boolean) {
  ...
  reactionAdded(id: string, timestamp: number, reaction: Reaction) {
    ...
    if (existing) {
      ++existing.count;
      if (reaction.reacted) existing.reacted = true;
    } else {
  ...
  reactionRemoved(
    id: string,
    timestamp: number,
    reaction: { name: string; reacted: boolean }
  ) {
    if (!this.isReceiving) return;
    const message = this.findMessage(id, timestamp);
    if (!message) return;
    ...
    if (reaction.reacted) existing.reacted = false;
    if (--existing.count === 0) message.reactions.splice(i, 1);
    this.onModifyMessage.dispatch(id, message);
    return message;
  }

  async readMessages() {
    if (this.messagesReady) return this.messages;
    ...
    // messages.
    if (this.account.status === "connected") this.messagesReady = true;
    else this.messages = [];
  ...
  foldMessages(messages: Array<Message>) {
  ...
  compareMessage(m: Message, pm: Message) {
    if (
      m.user.id == pm.user.id &&
      m.date.getTime() - pm.date.getTime() < 5 * 60 * 1000
    )
      m.isFolded = true;
    if (m.date.getDate() != pm.date.getDate()) m.setDayMarker();
