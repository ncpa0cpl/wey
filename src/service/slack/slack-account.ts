...
function filterChannel(c) {
  ...
  if (c.is_channel) return c.is_member;
  return true;
}
...
export class SlackAccount extends Account {
  ...
  async ready(data) {
    ...
    for (const m of members) {
      ...
      if (m.deleted !== true) this.saveUser(m);
    }
    // Current user.
  ...
  async updateEmoji() {
    ...
    for (const name in emoji) {
      ...
      if (url.startsWith("alias:")) this.emoji[name] = emoji[url.substr(6)];
      else this.emoji[name] = url;
    }
  }
  ...
  async updateChannels() {
    ...
    const { channels, groups, ims, mpims } =
      await this.rtm.webClient.users.counts(options);
  ...
  handleConnection() {
    ...
    if (this.isReconnect) this.updateChannels();
    // all connections are re-connect except for the first one.
    else this.isReconnect = true;
  }
    ...
    switch (event.subtype) {
      case "message_deleted": {
        if (event.previous_message.thread_ts) {
          ...
          if (thread) thread.deleteMessage(event.deleted_ts, event.deleted_ts);
        }
      ...
      case "message_changed": {
        ...
        if (
          ...
          const original = channel.findMessage(
            event.message.ts,
            event.message.ts
          );
      ...
      default: {
        ...
        if (message.threadId) {
          ...
          if (thread) thread.dispatchMessage(message);
        }
        // Emit event in channel if the message does not belong to a thread, or
        // if it is parent of a thread.
  ...
  setReaction(add, event) {
    ...
    if (add) {
      ...
      if (reaction.name == "+1" && reaction.reacted)
        channel.updateMessageStar(
          event.item.message.ts,
          event.item.message.ts,
          true
        );
    } else {
  ...
  async joinChannel(event) {
    ...
    if (event.type === "group_open") {
      const group = (
        await this.rtm.webClient.conversations.info({ channel: event.channel })
      ).channel;
  ...
  handlePresenceChange(event) {
    ...
    if (userChannel) userChannel.setAway(event.presence === "away");
  }
  ...
  async fetchUser(id: string, isBot?: boolean) {
  ...
  async listChannels(options, filter) {
    ...
    while (true) {
      ...
      let { channels, response_metadata } =
        await this.rtm.webClient.conversations.list(options);
  ...
  async reload() {
    if (this.status === "disconnected")
      this.rtm.start({ batch_presence_aware: true });
    else if (this.status === "connected") await this.updateChannels();
  }
  ...
  async leave(channel) {
    ...
    else await this.rtm.webClient.conversations.close({ channel: channel.id });
  }
}
