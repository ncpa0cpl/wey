...
import ChannelsSearcher from "./channels-searcher";
...
export class MainWindow {
  ...
  chatBox: ChatBox;
  channelsSearcher: ChannelsSearcher;
  selectedAccount: null | AccountsPanel;
  subscription: {
    onAddAccount: any;
    onRemoveAccount: any;
  };
  ...
  constructor() {
    ...
    this.subscription = {
      onAddAccount: accountManager.onAddAccount.add(
        this.addAccount.bind(this, true)
      ),
      onRemoveAccount: accountManager.onRemoveAccount.add(
  ...
  selectAccount(account) {
    ...
    if (this.selectedAccount)
      this.accountsPanel
        .findAccountButton(this.selectedAccount)
        .setActive(false);
    if (this.subscription.onUpdateConnection) {
      this.subscription.onUpdateConnection.detach();
    ...
    this.subscription.onUpdateConnection = account.onUpdateConnection.add(
  ...
  async showAllChannels(account) {
    ...
    if (
      this.channelsPanel.selectedChannelItem ||
      this.selectedAccount !== account
    )
      // may switch channel before await
      return;
  ...
  addAccount(focus, account) {
    ...
    if (focus) this.selectAccount(account);
  }
