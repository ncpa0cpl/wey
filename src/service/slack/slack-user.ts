import { parseEmoji } from "./message-parser";
...
export class SlackUser extends User {
  constructor(account: Account, event) {
    // Slack has multiple fields for names.
    let name = event.name;
    if (event.profile && event.profile.display_name)
      name = event.profile.display_name;
    else if (event.username) name = event.username;

    // And multiple fields for icons.
    let icon = "";
    if (event.icons) {
      if (event.icons.image_72) icon = event.icons.image_72;
    ...
    if (event.bot_id) {
      ...
      if (event.profile && event.profile.status_emoji)
        this.statusEmoji = parseEmoji(
          account,
          16,
          normalizeStatusEmoji(event.profile.status_emoji)
        );
    }
  }
